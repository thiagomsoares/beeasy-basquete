...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    if (config.textureUrl) {
      map = new TextureLoader().load(config.textureUrl);
      map.colorSpace = SRGBColorSpace;
    }

    const geometry = new SphereGeometry(1, 32, 32);
    const material = new Y({ envMap: envTexture, map, ...config.materialParams });
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, textureUrl, ...props }) => {
